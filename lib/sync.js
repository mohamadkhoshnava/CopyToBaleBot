...
import {
  getLink, S, allowed, dirOf, chatOf, bump, linkTitle, recordSent, claimEcho, isOurCopy,
} from 'lib/links';
...
async function syncPost(P) {
  ...
  // Jobs of a link run in order, so any copy we just sent is recorded by now.
  if (P.sp === 'bale' && !P.manual && await claimEcho(P.sc, P.msg)) return;
  ...
  const ids = await deliver(P.sp, dp, dc, prepared, {
    reply, silent: s.silent, bigNote: s.bigNote,
    onSent: (m, role) => recordSent(link.id, P.sp, P.sc, P.sm, dp, dc, m, role, prepared.kind),
  });
...
// P: { link_id, sp, sc, items: [msg], manual? }
async function syncAlbum(P) {
  const link = await getLink(P.link_id);
  if (!link) return;
  const dir = dirOf(P.sp);
  if (!P.manual && !allowed(link, dir)) return;
  let msgs = P.items.slice().sort((a, b) => a.message_id - b.message_id);
  if (P.sp === 'bale' && !P.manual) {
    const own = [];
    for (const m of msgs) if (await claimEcho(P.sc, m)) own.push(m.message_id);
    msgs = msgs.filter((m) => !own.includes(m.message_id));
  }
  const src = [];
  for (const m of msgs) {
    const c = extract(P.sp, m);
    if (!c) continue;
    const p = prepare(link, dir, c, { manual: P.manual });
    if (p) src.push({ m, c: p });
  }
  ...
  const dc = chatOf(link, dp);
  const s = S(link);
  const reply = P.manual ? undefined : await replyTarget(link, P.sp, P.sc, msgs[0]);
  await deliverAlbum(P.sp, dp, dc, src.map((x) => x.c), {
    reply, silent: s.silent, bigNote: s.bigNote,
    onSent: (m, role, i) => recordSent(link.id, P.sp, P.sc, src[i].m.message_id, dp, dc, m, role, src[i].c.kind),
  });
  await bump(link, dir);
  await statInc('sync_' + dir);
}
...
async function syncEdit(P) {
  ...
  if (await isOurCopy(P.sp, P.sc, P.sm)) return; // Bale echoing our own edit
