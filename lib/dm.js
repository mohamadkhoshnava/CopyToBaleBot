...
import {
  touchUser, getSession, setSession, clearSession, kvSetJSON,
} from 'lib/store';
...
async function forwardedPost(p, uid, m) {
  ...
  await kvSetJSON(`fw:${p}:${uid}:${m.message_id}`, { link_id: link.id, sp: p, sc: src.chatId, sm: src.msgId, msg: m, at: now() });
  const o = OTHER[p];
  const mapped = await counterpart(p, src.chatId, src.msgId);
  const html = `📨 این پست از کانال <b>${esc(p === 'tg' ? link.tg_title : link.bale_title)}</b> است.\n${mapped ? `✅ نسخهٔ آن در ${PNAME[o]} وجود دارد.` : `ℹ️ نسخه‌ای از آن در ${PNAME[o]} ثبت نشده.`}\n\nچه کاری انجام شود؟`;
  const kb = [
