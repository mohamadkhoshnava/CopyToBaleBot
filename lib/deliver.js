...
async function sendRich(dp, chatId, rich, params) {
  const m = await call(dp, 'sendMessage', { ...params, ...textParams(dp, rich) });
  return m.message_id;
}
...
export async function deliver(sp, dp, chatId, c, opt = {}) {
  ...
  const sendTail = async (r, params) => {
    for (const part of splitRich(r, LIMITS.text[dp])) ids.push(await sendRich(dp, chatId, part, ids.length ? rest : params));
  };
  ...
  if (c.kind === 'location') {
    const m = await call(dp, 'sendLocation', { ...first, ...c.extra });
    ids.push(m.message_id);
    if (rich.text) await sendTail(rich, rest);
    return ids;
  }
  if (c.kind === 'contact') {
    const m = await call(dp, 'sendContact', { ...first, ...c.extra });
    ids.push(m.message_id);
    return ids;
  }
...
export async function deliverAlbum(sp, dp, chatId, items, opt = {}) {
  ...
  if (groupable) {
    try {
      ...
      if (e.code === 429 || e.code === 403) throw e;
      console.warn('album send failed, falling back to single posts', e.description || e.message);
    }
  }
  const out = [];
  for (let i = 0; i < items.length; i++) {
    out.push(await deliver(sp, dp, chatId, items[i], i === 0 ? opt : { ...opt, reply: undefined }));
  }
  return out;
}
...
async function sendGroup(sp, dp, chatId, items, opt) {
  ...
  const out = items.map((_, i) => (list[i] ? [list[i].message_id] : []));
  for (const r of tails) {
    for (const part of splitRich(r, LIMITS.text[dp])) {
      const id = await sendRich(dp, chatId, part, baseParams(dp, chatId, { silent: opt.silent }));
      out[0].push(id);
    }
  }
  return out;
